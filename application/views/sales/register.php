						...
						else
						{
							foreach(array_reverse($cart, true) as $line=>$item)	
							{
								...
								<tr id="reg_item_top" bgcolor="#eeeeee" >
									...
									<td class="text text-success"><a href="<?php echo isset($item['item_id']) ? site_url('sales/view_item_modal/'.$item['item_id']) : site_url('sales/view_item_kit_modal/'.$item['item_kit_id']) ; ?>" data-toggle="modal" data-target="#myModal" ><?php echo H($item['name']); ?><?php echo $item['size'] ? ' ('.H($item['size']).')': ''; ?></a></td>
									...
									<ul class="list-unstyled">
										<li id="customer_view_modal"><strong><a href="<?php echo isset($customer_id) ? site_url('customers/view_modal/'.$customer_id) : site_url('customers/view_modal/'.$customer_id) ; ?>" data-toggle="modal" data-target="#myModal" ><?php echo character_limiter(H($customer), 25); ?></a></strong>
											<?php if ($this->config->item('customers_store_accounts') && isset($customer_balance)) {?>
												(<?php echo lang('customers_balance').': '.$customer_balance; ?>)
										<?php } ?>
										</li>
...
<a href="#" id="opener" class="visible-lg"><?php echo lang('sales_keyboard_help_title');?></a>
