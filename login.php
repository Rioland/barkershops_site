...
class Login extends CI_Controller 
{
		...
		else
		{
			...
			if($this->form_validation->run() == FALSE)
			{
				...
				if (isset($db['site']))
				{
					...
					{
						if ($this->_is_subscription_cancelled_within_14_days($site_db))
						{
							$data['subscription_cancelled_within_14_days']  = FALSE;
	...
	function _is_subscription_cancelled_within_14_days($site_db)
	{
		$phppos_client_name = substr($_SERVER['HTTP_HOST'], 0, strpos($_SERVER['HTTP_HOST'], '.'));
		$thirty_days_ago = date('Y-m-d H:i:s', strtotime("now -14 days"));
		$site_db->select('subscr_status');	
		$site_db->from('subscriptions');	
		$site_db->where('username',$phppos_client_name);
		$site_db->where('subscr_status','cancelled');
